...
from google.appengine.api import datastore, namespace_manager
...
class CloudStorageStub(object):
    ...
    try:
      ...
      if content:
        ...
        if start > gcs_file.next_offset:
          ...
          self.blob_storage.StoreBlob(blobkey, BytesIO(content))
  ...
  def _end_creation(self, token, _upload_filename):
    ...
    self.blob_storage.StoreBlob(token, BytesIO(content))
