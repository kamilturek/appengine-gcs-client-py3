...
class FilenameEscapingTest(unittest.TestCase):
    ...
    def testEscaping(self):
        ...
        EscapeUnescapeFilename(
            "/bucket/a ;/?:@&=+$,", "/bucket/a%20%3B/%3F%3A%40%26%3D%2B%24%2C"
        )
...
class RetryParamsTest(unittest.TestCase):
    ...
    def testMinRetries(self):
        start_time = time.time()
        retry_params = api_utils.RetryParams(
            min_retries=3, max_retry_period=10, initial_delay=1
        )
    ...
    def testPerRequestSetting(self):
        ...
        self.assertEqual(api_utils.RetryParams(), api_utils._get_default_retry_params())
...
class RetriableTaskletTest(unittest.TestCase):
    ...
    def testTaskletWasSuccessful(self):
        fut = api_utils._RetryWrapper(api_utils.RetryParams()).run(tasklet1, a=1, b=2)
        a, b = fut.get_result()
    ...
    def testRetryDueToBadResult(self):
        fut = api_utils._RetryWrapper(
            ...
            should_retry=lambda r: r < 0,
        ).run(self.tasklet_for_test, results=[-1, -1, 1])
        r = fut.get_result()
    ...
    def testRetryReturnedABadResult(self):
        fut = api_utils._RetryWrapper(
            api_utils.RetryParams(min_retries=1, max_retries=3),
            should_retry=lambda r: r < 0,
        ).run(self.tasklet_for_test, results=[-1, -1, -1, -1])
        r = fut.get_result()
        self.assertEqual(-1, r)
    ...
    def testRetryDueToTransientError(self):
        ...
        ).run(self.tasklet_for_test, results=results)
        r = fut.get_result()
        self.assertEqual(1, r)
    ...
    def testRetryDueToError(self):
        ...
        ).run(self.tasklet_for_test, results=[ValueError, ValueError, ValueError, 1])
        r = fut.get_result()
        self.assertEqual(1, r)
    ...
    def testTooLittleRetry(self):
        fut = api_utils._RetryWrapper(
            ...
            retriable_exceptions=(ValueError,),
        ).run(self.tasklet_for_test, results=[ValueError, ValueError])
    ...
    def testNoRetryDueToRetryParams(self):
        ...
        fut = api_utils._RetryWrapper(
            retry_params, retriable_exceptions=[ValueError]
        ).run(tasklet3, a=1)
        self.assertRaises(ValueError, fut.get_result)

    def testNoRetryDueToErrorType(self):
        fut = api_utils._RetryWrapper(
            api_utils.RetryParams(), retriable_exceptions=[TypeError]
        ).run(tasklet3, a=1)
        self.assertRaises(ValueError, fut.get_result)

    def testRuntimeError(self):
        fut = api_utils._RetryWrapper(api_utils.RetryParams()).run(tasklet4)
